...
router.post("/checkout", auth, async (req, res) => {
  try {
    ...
    for (let i = 0; i < cart.length; i++) {
      const product = await Product.findById(cart[i]._id, "price");
      subTotal = subTotal + Number(product.price) * cart[i].quantity;
    }
...
router.post("/paypal", auth, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      product.sales = product.sales + 1;
      console.log(product.salesPerMonth.some((el) => el.month === month));
      if (product.salesPerMonth.some((el) => el.month === month)) {
        product.salesPerMonth.forEach((el) => {
          if (el.month === month) {
            el.sales = el.sales + 1;
